...
<?php if ($_keywords): ?>
<meta property="article:tag"    content="<?= htmlspecialchars($_keywords) ?>">
<?php endif; ?>
<?php endif; ?>

<!-- Keywords (tags) -->
<?php if ($_keywords): ?>
<meta name="keywords" content="<?= htmlspecialchars($_keywords) ?>, தமிழ் செய்தி, Tamil News, <?= htmlspecialchars($_siteName) ?>">
<?php else: ?>
<meta name="keywords" content="தமிழ் செய்தி, Tamil News, Latest Tamil News, தமிழ்நாடு செய்திகள்">
<?php endif; ?>

<!-- Google News / News Article meta -->
<?php if ($_isArticle): ?>
...
<?php if ($_isArticle): ?>
{
  ...
  "@type": "<?= $article['youtube_video_id'] ? 'VideoObject' : 'NewsArticle' ?>",
  "headline": <?= json_encode($article['title'] ?? '', JSON_UNESCAPED_UNICODE) ?>,
  "description": <?= json_encode(mb_substr(strip_tags($article['excerpt'] ?? ''), 0, 200), JSON_UNESCAPED_UNICODE) ?>,
  "url": <?= json_encode($_canonical) ?>,
  <?php if ($_ogImage): ?>"image": <?= json_encode($_ogImage) ?>,<?php endif; ?>
  "datePublished": <?= json_encode($_pubDate) ?>,
  "dateModified":  <?= json_encode($_modDate) ?>,
  "author": {
    "@type": "Person",
    "name":  <?= json_encode($_author, JSON_UNESCAPED_UNICODE) ?>
  },
  "publisher": {
    "@type": "Organization",
    "name":  <?= json_encode($_siteName, JSON_UNESCAPED_UNICODE) ?>,
    "logo": {
      "@type": "ImageObject",
      "url": <?= json_encode($_siteUrl . '/assets/images/logo.png') ?>
    }
  },
  "inLanguage": "ta",
  "keywords": <?= json_encode($_keywords, JSON_UNESCAPED_UNICODE) ?>,
  "articleSection": <?= json_encode($article['category_name'] ?? '', JSON_UNESCAPED_UNICODE) ?>
  <?php if ($article['youtube_video_id']): ?>,
  "embedUrl": "https://www.youtube.com/embed/<?= htmlspecialchars($article['youtube_video_id']) ?>",
  "thumbnailUrl": "https://img.youtube.com/vi/<?= htmlspecialchars($article['youtube_video_id']) ?>/hqdefault.jpg"
  <?php endif; ?>
}
<?php else: ?>
{
  ...
  "@type": "WebSite",
  "name":  <?= json_encode($_siteName, JSON_UNESCAPED_UNICODE) ?>,
  "url":   <?= json_encode($_siteUrl . '/') ?>,
  "inLanguage": "ta",
  "potentialAction": {
    "@type": "SearchAction",
    "target": {
      "@type": "EntryPoint",
      "urlTemplate": <?= json_encode($_siteUrl . '/search?q={search_term_string}') ?>
    },
...
<header class="header">
  ...
  <div class="header-top">
    <div class="logo-block">
      <a href="<?= $baseUrl ?>/public/" class="vel-logo-link">
        <div class="vel-brand-wrap">
          <!-- Logo image — place your VS logo at public/assets/images/vel-logo.png -->
          <?php if (file_exists(dirname(__DIR__,2).'/public/assets/images/vel-logo.svg')): ?>
          <img src="<?= $r ?>/assets/images/vel-logo.png" class="vel-img" alt="வேள் சுடர்">
          <?php else: ?>
          <!-- Placeholder monogram until image is ready -->
    ...
    <div class="header-meta">
      ...
      <div>Digital News Portal</div>
    </div>
    <form class="search-bar" action="<?= $baseUrl ?>/public/search" method="GET">
      <input type="text" name="q" placeholder="தேடு..." value="<?= htmlspecialchars($_GET['q'] ?? '') ?>">
      <button type="submit">🔍</button>
    </form>
    <?php if ($reader): ?>
    <div class="user-avatar-wrap" id="userAvatarWrap" style="display:block">
      <?php if ($reader['avatar']): ?>
      <img src="<?= htmlspecialchars($reader['avatar']) ?>" class="user-avatar-img" onclick="toggleDropdown()" alt="">
      <?php else: ?>
      <div class="user-avatar" onclick="toggleDropdown()"><?= strtoupper(substr($reader['name'], 0, 1)) ?></div>
      <?php endif; ?>
    ...
    <?php else: ?>
    <button class="login-btn" id="loginBtn" onclick="openModal()">
  ...
  <nav class="nav">
    <div class="nav-inner">
      <a href="<?= $baseUrl ?>/public/" class="nav-link <?= (parse_url($_SERVER['REQUEST_URI'],PHP_URL_PATH) === parse_url($baseUrl,PHP_URL_PATH).'/public/' || parse_url($_SERVER['REQUEST_URI'],PHP_URL_PATH) === parse_url($baseUrl,PHP_URL_PATH).'/public') ? 'active' : '' ?>">முகப்பு</a>
...
<!-- MOBILE SEARCH ROW (below header, mobile only) -->
<div class="mobile-search-row">
  <form class="search-bar" action="<?= $baseUrl ?>/public/search" method="GET" style="width:100%">
    <input type="text" name="q" placeholder="செய்திகளை தேடுங்கள்..." value="<?= htmlspecialchars($_GET['q'] ?? '') ?>">
    <button type="submit">🔍</button>
  </form>
</div>
  ...
  <div class="footer-inner">
    <div class="footer-top">
      <div>
        ...
        <div class="footer-desc">தமிழ்நாட்டின் நம்பகமான டிஜிட்டல் செய்தி தளம். உடனடி செய்திகள், உண்மையான பத்திரிகையியல்.</div>
      </div>
      <div>
...
<div class="modal-overlay" id="loginModal" onclick="handleOverlayClick(event)">
  <div class="modal-box">
    <div class="modal-header">
      ...
      <div class="vel-logo" style="font-size:22px;justify-content:center"><span class="vel-word1">வேள்</span><span class="vel-word2">சுடர்</span></div>
...
<script>
/* ── வேள் சுடர் — Header Canvas Animation ── */
(function() {
  ...
  class Particle {
    ...
    reset() {
      ...
      this.hue    = Math.random() < 0.7 ? 0 : 30; // red or orange
    }
    update() {
      this.x    += this.speedX;
      this.y    += this.speedY;
    ...
    draw() {
      ...
      ctx.fillStyle   = this.hue === 0
        ? `rgba(220,30,30,${this.life})`
        : `rgba(255,140,0,${this.life})`;
      ctx.shadowColor = this.hue === 0 ? '#C0001A' : '#FF8C00';
      ctx.shadowBlur  = 6;
      ctx.beginPath();
